...
The `plot_devmodels()` function facilitates quick visualizations of predictions of the Thermal Performance Curves to guide model selection under ecological criteria. It takes as an input the resulting `tibble`/`data.frame` stored as an object from `fit_devmodels()`. It has also arguments to title and subtitle the figure with the species' name and the life stage evaluated, respectively.
