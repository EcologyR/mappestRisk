...
aphid_risk_map <- ggplot2::ggplot() +
  tidyterra::geom_spatvector(data = Bhutan_sf, # <- countries background
                   fill = "gray85") +
...
ggplot2::ggplot() +
  ...
  tidyterra::geom_spatraster_contour_filled(data = risk_rast_bhutan_epsg4326,
                                            breaks = seq(0, 12),
                                            maxcell = Inf) +
  ...
  theme(legend.position = "bottom") +
  theme(plot.title = element_text(face = "bold")) +
  geom_sf(data = china_sf, fill = "black",
          color = NA) +
  geom_sf(data = india_sf, fill = "black",
          color = NA)+
