...
worldmap_sf <- rnaturalearth::ne_countries(scale = 10,
                                           type = "countries",
                                           returnclass = "sf") |>
  sf::st_transform(4326) # <- from default EPSG: 4326 to target EPSG:3035
```
...
noNA_risk_rast <- risk_layer_bhutan |> 
  tidyterra::drop_na()
risk_rast_bhutan_epsg4326 <- terra::project(x = noNA_risk_rast,
                                               "EPSG:4326")
bbox_rast <- sf::st_bbox(risk_rast_bhutan_epsg4326)
```
...
```{r}
max_risk_bhutan<- terra::minmax(risk_rast_bhutan_epsg4326)[2] # for manual scale fill visualization 
...
aphid_risk_map <- ggplot2::ggplot() +
  ...
  tidyterra::geom_spatraster(data = risk_rast_bhutan_epsg4326, 
                             maxcell = Inf) +
