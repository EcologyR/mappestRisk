...
#' 'SpatVector' polygon map (obtained with [terra::vect()]); or a 'SpatExtent'
#' object (obtained with [terra::ext()]); or a numeric vector of length 4
#' specifying the region coordinates in the order xmin, xmax, ymin, ymax;
#' or a character vector of country name(s) in English. If NULL, the output maps #' will cover the entire `t_rast` if provided, or the entire world otherwise.
...
map_risk <- function(t_vals = NULL,
  ...
  t_rast_crop[outside] <- ifelse(output == "binary", 0, NA_real_)
