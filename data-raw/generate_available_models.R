...
available_models <- dplyr::tibble(model_name = model_names) |>
  dplyr::mutate(package = dplyr::if_else(
    model_name %in% c("briere1",  "lactin1",  #
                      "wang", "regniere",
                      "mod_polynomial"),
    "devRate",
    "rTPC"),
  ...
  dplyr::mutate(formula = dplyr::case_when(
    ...
    #model_name == "flextpc" ~ "rTPC::flextpc_2024(temp, tmin, tmax,
    #                                              rmax, alpha, beta)",
  ...
  dplyr::mutate(fitting_formula = dplyr::case_when(
